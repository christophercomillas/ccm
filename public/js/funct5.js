...
$(document).ready(function() {  
    ...
    $('#checkdate,#checkexpdate').mask("00/00/0000");
    ...
    $('button#insimport').click(function(){
        //console.log($(this).text());
        $(this).prop('disabled',true);
        $(this).html('<span class="fa fa-spinner fa-spin"></span> Updating..');

        streaminstitutionalimport('institutionalimport');        
    });
    ...
    $("body").on('change', "input#checkdate", function(){
        ...
        checkType(cdate);
    }).on('change', "input#checkexpdate", function(){
        var cdate = $(this).val();
        ...
        BootstrapDialog.show({
            ...
            buttons: [{
                ...
                action:function(dialogItself){   
                    ...
                    var checkexpdate = $('#checkexpdate').val();           
                    ...
                    {
                        hasError = true;
                        $('select#cclass').parent().addClass('has-error');                    
                        $( "select#cclass" ).after( "<span class='help-block'><strong>Please enter Check Class.</strong></span>" );
                    }

                    if(ccategory.trim()=="")
                    {
                        hasError = true;
                        $('select#ccategory').parent().addClass('has-error');                    
                        $( "select#ccategory" ).after( "<span class='help-block'><strong>Please enter Check Class.</strong></span>" );
                    }

                    if(currency.trim()=="")
                    {
                        hasError = true;
                        $('select#currency').parent().addClass('has-error');                    
                        $( "select#currency" ).after( "<span class='help-block'><strong>Please enter Check Class.</strong></span>" );
                    }
                    ...
                    if(checkexpdate.trim() != "")                    
                    {
                        if(!isValidDate(checkexpdate))
                        {
                            hasError = true;
                            $('input#checkexpdate').parent().addClass('has-error');                    
                            $( "input#checkexpdate" ).after( "<span class='help-block'><strong>Expiration Date is invalid.</strong></span>" );                       
                        }
                    }
                                    ...
                                    {    
                                        ...
                                        {                                            
                                            ...
                                            var newdata = [
                                                ...
                                                bankname,   
                                                checkamt,   
                                                '<div class="action-check" data-id="'+key+'">&emsp;<a href="#" title="SHOW"><span class="glyphicon glyphicon-list" id="showcheck"></span></a>&emsp;<a href="#" title="EDIT" ><span class="glyphicon glyphicon-edit" id="editcheck"></span></a></div>'];
                                                // &emsp;<a href="#" title="REMOVE" ><span class="glyphicon glyphicon-remove-sign" id="removecheck"></span></a>
                                    ...
                                    {           
                                        if(data.status)
                                        {
                                            ...
                                            t.row.add( [                    
                                                ...
                                                bankname,   
                                                checkamt,                                               
                                                '<div class="action-check" data-id="'+data.checkkey+'">&emsp;<a href="#" title="SHOW"><span class="glyphicon glyphicon-list" id="showcheck"></span></a>&emsp;<a href="#" title="EDIT" ><span class="glyphicon glyphicon-edit" id="editcheck"></span></a></div>'
                                            ] ).draw( false );
                                            dialogItself1.close();
                                            // &emsp;<a href="#" title="REMOVE" ><span class="glyphicon glyphicon-remove-sign" id="removecheck"></span></a>
                                            // t.row.add( [                 
    ...
    $("body").on('click', "table#checklist tbody tr td a span#tagasbounced, span#tagasbounced", function(){
        event.preventDefault();      
        var $this = $(this);         
        var id = $(this).closest('div.action-check').attr('data-id');
        var checkno = $(this).closest('div.action-check').attr('data-check');
        console.log("key="+id);
        // var key = $(this).parent().text();
        // return false;
        if(id.trim()=='')
        ...
        var r = confirm("Tag as Bounced Check #"+checkno);
        if (r == true) {
            //dri
            $.ajax({
                url:siteurl+'/taggedasbounced/'+id,
                success:function(data)
                {
                    console.log(data);
                    if(data.status)
                    {
                        ...
                        alert('Check #'+checkno+' successfully deleted.');
                        window.location.reload();                        
                    }
                    else 
                    {
    ...
    $("body").on('click', "table#checklist tbody tr td a span#viewcheck,span#viewcheck", function(){
        var id = $(this).closest('div.action-check').attr('data-id');
        console.log("key="+id);
        // var key = $(this).parent().text();
        // return false;
        ...
        BootstrapDialog.show({
            ...
            data: {
                'pageToLoad': siteurl+'/viewcheckdialog/'+id,
            },
            cssClass: 'addnewcheck',
            onshow: function(dialog) {
                // dialog.getButton('button-c').disable();
    ...
    $("body").on('click', "table#checklist tbody tr td a span#updatepdc,span#updatepdc", function(){

        var id = $(this).closest('div.action-check').attr('data-id');
        var type = $(this).closest('div.action-check').attr('data-type');
        ...
        BootstrapDialog.show({
            title: '<span class="glyphicon glyphicon-edit"></span> Update PDC',
            closable: true,
            closeByBackdrop: false,
            closeByKeyboard: true,
            message: function(dialog) {
            ...
            data: {
                'pageToLoad': 'updatebounced/'+id+'/'+type,
            },
            ...
            buttons: [{
                ...
                action:function(dialogItself){   
                    ...
                    if(updatetype=='replacement')
                    {
                        ...
                        if($('#replacetype').val()=='CHECK AND CASH') 
                        {
                            if(repcash.trim()=='' || repcash=='0.00' || repcash=='0')
                            {
                                $('.response-dialog').html("<div class='alert alert-danger mb-0'>Cash Amount is not valid.</div>");
                                return false;
                            }
                        }
                    ...
                    BootstrapDialog.show({
                        ...
                        buttons: [{
                            ...
                            action:function(dialogItself1){
                                //dri
                                $('.response-dialog').html('');
                                //url:siteurl+'/deletecheck/'+id,
                                $.ajax({
                                    url:siteurl+'/updatepdc',
                                    data:formDATA,
                                    type:'POST',
                                    success:function(data)
                                    {    
                    ...
                    BootstrapDialog.show({
                        ...
                        buttons: [{
                            ...
                            action:function(dialogItself1){
                                ...
                                else if(updatetype =='replacement')
                                {
                                    $('#tagbouncecid-'+id).val(true);
    ...
    $('.checkboxItemsDep').click(function(){
        ...
        $.ajax({
            url:siteurl+'/checkexist/'+id+'/'+exist,
            success:function(data)
            {
    ...
    $("body").on('click', "table#checklist tbody tr td a span#deletecheckforclearing,span#deletecheckforclearing", function(){
        var $this = $(this);
        var id = $(this).closest('div.action-check').attr('data-id');
        var checkno = $(this).closest('div.action-check').attr('data-check');
        var r = confirm("Remove Check #"+checkno);
        if (r == true) {
            //dri
            $.ajax({
                url:siteurl+'/deletecheck/'+id,
    ...
    $("body").on('change', "select#replacetype", function(){
        var type = $(this).val();
       
        if(type=='CHECK'){
            $('.repcashdiv').addClass('divhide');
        }
        else 
        {
            $('.repcashdiv').removeClass('divhide');
        }
    });
    ...
    $('#report').click(function(){
        ...
        $('.response').html('');
        if(date.trim()==""){
            $('.response').html('<div class="alert alert-danger">Please input date.</div>');
            $('#report').prop("disabled",false);
            return;
        }
		$.ajax({
			url:siteurl+'/getcheck',
			data:{date:date},
			type:'POST',			
			beforeSend:function(){									
			},
			success:function(data){	
				...
				if(data.st)
				{								
					window.location.href = siteurl+'/getcheckreport/'+date;
				}
				else 
				{

                    $('.response').html('<div class="alert alert-danger">No data exist.</div>');
                    $('#report').prop("disabled",false);
				}
			}
		});
        ...
        //window.location.url = siteurl+'/report/''
    });
...
function calcAmountsBounceCleared()
{
    ...
    $('span.forclearexist').text(totexist);   
...
function streaminstitutionalimport(url){
    $.ajax({
        url: url,
        dataType: 'text',
        cache: false,
                    ...
                    try{
                        ...
                        if (output.readyState == 3) {
                            ...
                            if(result.status=='noupdate')
                            {
                                ...
                                setTimeout(function(){
                                    $('button#insimport').html('<span class="fa fa-refresh"></span> Update');
                                },800)
                            }     
                            ...
                            if(result.status=='complete')
                            {
                                ...
                                $( "p.updatestatus" ).text( result.message );
                                setTimeout(function(){
                                    $('button#insimport').html('Data Imported');
                                },800)
                            }
